...
"filther", "phylo", "use_memory", "recieveEcho",\
...
void shell()
{
    ...
    while (!exit)
    {
		...
		if(fn == phyloFunc){
			printf("ES PHYLO");	
			parsed.isBackground = 1;

			char* name = "phyloCounter";
		    void (*fn2)(uint8_t, char **) = phyloCounter;
			char* argv[] = {"phyloCounter"};
			pid2 = createProcess(fn2, 1, argv, name);
			fgProccess = pid2;
		}
